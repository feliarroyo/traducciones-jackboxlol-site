...
import { CREDITS_DONORS, CREDITS_REGISTRY_ADMINS, CREDITS_REGISTRY_EXTRAS, CREDITS_REGISTRY_GAMES } from "../../data/creditsRegistry";
import { DUB_CREDITS_REGISTRY } from "../../data/dubCreditsRegistry";
import CreditCardContainer from "../cards/CreditCardContainer";
import CreditCardAdminContainer from "../cards/CreditCardAdminContainer";
import DubGameCardXL from "../cards/DubGameCardXL";
import DubGameCard from "../cards/DubGameCard";
import CreditCardList from "../cards/CreditCardList";
import CreditCardText from "../cards/CreditCardText";
...
export default function CreditsPage() {
    ...
    const { mainDubs, minorDubs, comingSoonDubs } = useMemo(() => {
        return {
            mainDubs: DUB_CREDITS_REGISTRY.filter((g) => g.category === "mainDub"),
            minorDubs: DUB_CREDITS_REGISTRY.filter((g) => g.category === "minorDub"),
            comingSoonDubs: DUB_CREDITS_REGISTRY.filter(
                (g) => g.category === "comingSoon" || g.category === "comingSoonMinor"
            ),
        };
    }, []);
                            ...
                            <>
                                ...
                                <section className="space-y-6">
                                    ...
                                    <div className="flex flex-row flex-wrap gap-6 justify-center items-center w-full mx-auto">
                                        {mainDubs.map((game) => (<DubGameCardXL key={game.id} game={game} />))}
                                    </div>
                                </section>
                                ...
                                <section className="space-y-6">
                                    ...
                                    <div className="flex flex-row flex-wrap gap-4 justify-center items-start w-full mx-auto">
                                        {minorDubs.map((game) => (<DubGameCard key={game.id} game={game} />))}
                                    </div>
                                </section>
                                <section className="space-y-6">
                                    <h2 className="text-2xl font-extrabold text-slate-200 border-b border-slate-800 pb-2">
