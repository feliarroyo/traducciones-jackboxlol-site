    ...
    return (
        ...
        >
            ...
            <div className="w-full h-18 relative opacity-60 hover:opacity-100 transition-opacity shrink-0">
                <Image src={packLogoSrc} alt={packLogoAlt} fill className="object-contain fill draggable={false} pointer-events-none" />
            </div>
            ...
            <div className="flex flex-row items-center justify-center gap-6 w-full py-2">
                {gameLogos.map((game, index) => (
                    ...
                    >
                        <Image src={GAME_ASSETS[game]?.src} alt={GAME_ASSETS[game]?.alt} fill className="object-contain" draggable={false} pointer-events-none/>
                    </div>
                ))}
            </div>
